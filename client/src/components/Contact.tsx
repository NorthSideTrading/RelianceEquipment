    ...
    <section id="contact" className="py-20 bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="bg-red-600/20 border border-red-500 rounded-xl p-8 text-center mb-16"
        >
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {contactItems.map((item, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="text-center bg-white/5 backdrop-blur-sm rounded-xl p-8 hover:bg-white/10 transition-all duration-300 hover-lift"
            >
              <div className="w-16 h-16 bg-red-600 rounded-full flex items-center justify-center mx-auto mb-6">
                <item.icon className="h-8 w-8 text-white" />
              </div>
              <h3 className="text-2xl font-bold text-white mb-4 tracking-wider">{item.title}</h3>
